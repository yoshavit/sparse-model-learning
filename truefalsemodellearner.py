...
class TrueFalseModelLearner(ModelLearner):
        ...
        for t in range(1, max_prediction_length + 1):
            for t0 in range(max_prediction_length - t + 1):
                ...
                state_prediction_loss = self.build_cluster_loss(
                    xf, xf_hat, self.input_is_valid, pos_weight_multiplier)
                ...
                if with_reward_terminal:
                    ...
                    terminal_loss = tf.reduce_mean(tf.boolean_mask(
                        tf.nn.sigmoid_cross_entropy_with_logits(
                            labels=tf.cast(tfm1, tf.float32),
                            logits=tfm1_hat_logits), self.input_is_valid))
    ...
    def build_pair_cluster_executer(self, n_steps):
        ...
        encoded_end_state = self.build_encoder(end_state)
        ac_sequence = self.input_actions[:n_steps]
        approx_end_state = self.get_future_encoded_state(
    ...
    def build_cluster_loss(self, input1, input2, is_same, pos_weight_multiplier):
    ...
    def gather_gameplay_data(self, env, num_games, policy="random"):
        ...
        for i in range(num_games):
            ...
            while not done and c < GAMEPLAY_TIMOUT:
                ...
                game_memory.append((obs, action, new_obs, rew, bool(done)))
                obs = new_obs
