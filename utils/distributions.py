...
import numpy as np
import mcts_utils.tf_util as U
...
class BernoulliPd(Pd):
    ...
    def logp(self, x):
        return - U.sum(tf.nn.sigmoid_cross_entropy_with_logits(self.logits, tf.to_float(x)), axis=1)
    def kl(self, other):
        return U.sum(tf.nn.sigmoid_cross_entropy_with_logits(other.logits, self.ps), axis=1) - U.sum(tf.nn.sigmoid_cross_entropy_with_logits(self.logits, self.ps), axis=1)
    def entropy(self):
        return U.sum(tf.nn.sigmoid_cross_entropy_with_logits(self.logits, self.ps), axis=1)
    def sample(self):
